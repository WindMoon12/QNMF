...
epochs = 10, bs = 20, optimizer = None, criterion = None):  # outdir = None, file_prefix = None):
    ...
    for epoch in range(epochs):
        ...
        for i, sampled_batch in enumerate(train_loader):
            ...
            print('batch({}):{:.4f}: cost_time'.format(i,time.time()-t2))
        ...
        with open('./result/result_train.txt', 'a+') as f:
            f.write('Epoch:{}, TR_Loss: {:.4f}, TR_Acc: {:.4f}\n'.format(epoch, tr_loss, tr_acc))
        ...
        with open('./result/result_test.txt', 'a+') as f:
            f.write('Epoch: {} VAL_Loss: {:.4f}, VAL_Acc: {:.4f}\n'.format(epoch, val_loss, val_acc))
